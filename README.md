...
把一本《封神演義》原文(維基文庫),自動變成可瀏覽、可查證的 wiki 資料庫。
姊妹作:[西遊記 Wiki](https://github.com/pondahai/xiyouji-wiki)、
[三國演義 Wiki](https://github.com/pondahai/sanguo-wiki)(方法論與開發歷程詳見三國版 README)。
...
- **靜態網站**:線上直接看 <https://pondahai.github.io/fengshen-wiki/>,
  push 後由 CI 自動從 vault 重建部署 GitHub Pages;
  也可本地跑 `python scripts/build_html.py` 後開 `site/index.html` 離線瀏覽

## 與西遊記版的差異

本 repo 直接複製 [xiyouji-wiki](https://github.com/pondahai/xiyouji-wiki)(config 化架構),
只改了 `scripts/config.py` 與 `scripts/characters.py`,並以 `fetch_wikisource.py`
取代 `epub_to_txt.py`(原文改抓[維基文庫](https://zh.wikisource.org/zh-hant/封神演義),
逐卷下載 zh-hant 頁面、`<p>` 段落轉純文字)。

另外封神演義的回目是單句(「第一回 紂王女媧宮進香」),不是西遊記的上下聯,
`build_wiki.py` 的標題解析改為串接 regex 所有非空群組,兩種格式通用。

## 處理流程

```mermaid
flowchart TD
    A[維基文庫 zh-hant] -->|"fetch_wikisource.py(不用LLM)"| B[fengshen_raw.txt]
...
```bash
python scripts/fetch_wikisource.py   # 0. 維基文庫 → data/fengshen_raw.txt(分鐘級)
...
《封神演義》原文為公有領域;文字取自[維基文庫](https://zh.wikisource.org/zh-hant/封神演義)。腳本部分隨意使用。
